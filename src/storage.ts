...
export type Application = { id: string; job: Job; status: 'revisada' | 'preparando' | 'autorizada'; updatedAt: string };
...
export function saveApplication(job: Job, status: Application['status'] = 'revisada') { const current = loadApplications(); const existing = current.find(a => a.job.id === job.id); const next = existing ? current.map(a => a.job.id === job.id ? { ...a, status, updatedAt: new Date().toISOString() } : a) : [{ id: crypto.randomUUID?.() ?? `${Date.now()}-${job.id}`, job, status, updatedAt: new Date().toISOString() }, ...current]; write(APPLICATIONS_KEY, next); return next; }
